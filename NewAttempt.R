...
input_path <- "MBAM_WP/"
data <- read_excel(file.path(input_path, "Actuals_ccar2025.xlsx"))  # Actuals must include VIX_diff_sqrtd & MMD_MUNI_30Y_AAA_YIELD_Diff
...
# 3. Extract coefficients (including intercept/drift)
phi1 <- coef(m2)["ar1"]
b0   <- coef(m2)["intercept"]
...
for (i in seq_len(h)) {
  dy_new         <- phi1 * dy_prev + b0 + b1 * x1_future[i] + b2 * x2_future[i]
  y_new          <- y_prev + dy_new
  manual_base[i] <- y_new
  dy_prev        <- dy_new
  y_prev         <- y_new
